...
void MainWindow::on_pbsignin_clicked()
{
    ...
    if(q.next()){
        ...
        //向表room_state插入记录
        QSqlQuery insq;
        insq.prepare("insert into room_state values(:rid,:u_id,null,0,0,:time)");
        insq.bindValue(":r_id",room_id);
        insq.bindValue(":u_id",user_id);
        insq.bindValue(":time",check_in_time);
        insq.exec();
    }
...
void MainWindow::on_pbsignout_clicked()
{
    ...
    QString user_id = ui->leuserpwd->text().trimmed();
    QSqlQuery q;
    q.prepare("delete from room_state where room_id=:rid and user_id=:uid");
    q.bindValue(":rid",room_id);
